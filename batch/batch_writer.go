...
func NewBatchWriter[T any](client *elasticsearch.Client, index string, retryAll bool, opts ...func(*T)) *BatchWriter[T] {
	return NewBatchWriterWithIdName[T](client, index, retryAll, "", opts...)
}
func NewBatchWriterWithIdName[T any](client *elasticsearch.Client, index string, retryAll bool, idFieldName string, opts ...func(*T)) *BatchWriter[T] {
	var t T
	...
	return &BatchWriter[T]{client: client, index: index, idx: idx, FieldMap: es.BuildMap(modelType), Map: mp, retryAll: retryAll}
}
func (w *BatchWriter[T]) Write(ctx context.Context, objs []T) ([]int, error) {
	le := len(objs)
	if le <= 0 {
		return nil, nil
	}
	...
	return SaveBatch(ctx, w.client, w.index, models)
}
