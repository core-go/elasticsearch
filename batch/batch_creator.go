...
func NewBatchCreator[T any](client *elasticsearch.Client, index string, retryAll bool, opts ...func(*T)) *BatchCreator[T] {
	return NewBatchCreatorWithIdName[T](client, index, retryAll, "", opts...)
}
func NewBatchCreatorWithIdName[T any](client *elasticsearch.Client, index string, retryAll bool, idFieldName string, opts ...func(*T)) *BatchCreator[T] {
	var t T
...
func (w *BatchCreator[T]) Write(ctx context.Context, objs []T) ([]int, error) {
	...
	return CreateBatch(ctx, w.client, w.index, models)
}
